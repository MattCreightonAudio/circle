...
# Amplitudes
initial_A_avg = 0.5     # Average Amplitude
initial_A_R = 1.0       # Amplitude Ratio (A2 / A1)
initial_S1 = 0.0        # Shape 1 (S = (Al - Ar) / (Al + Ar), +1 = CCW)
initial_S2 = 0.0        # Shape 2

# Phases
initial_theta = 0.0
initial_tau = -np.pi / 4.0 # Makes E1 internal phase diff (phi_l1 - phi_r1) = pi/2
initial_phi = 0.0
initial_Psi = 0.0
...
# Amplitudes (Model A)
# SIGN FLIP APPLIED HERE: Ar is proportional to (1 - S), Al is proportional to (1 + S)
...
ax_W_r = plt.axes([0.15, v_pos - v_step, 0.65, 0.03])

# Amplitudes (4)
v_pos -= (2 * v_step + 0.02) # Add gap
ax_A_avg = plt.axes([0.15, v_pos, 0.65, 0.03])
...
# Amplitudes
slider_A_avg = Slider(ax=ax_A_avg, label='$A_{avg}$ (Avg Amp)', valmin=0.0, valmax=1.5, valinit=initial_A_avg) 
...
slider_theta = Slider(ax=ax_theta, label=r'$\theta$ (theta)', valmin=-np.pi, valmax=np.pi, valinit=initial_theta)
slider_tau = Slider(ax=ax_tau, label=r'$\tau$ (tau)', valmin=-np.pi, valmax=np.pi, valinit=initial_tau)
slider_phi = Slider(ax=ax_phi, label=r'$\phi$ (phi)', valmin=-np.pi, valmax=np.pi, valinit=initial_phi)
...
def update(val):
    ...
    # Amplitudes (Model A)
    # SIGN FLIP APPLIED HERE: Positive S means larger Al (CCW) and smaller Ar (CW)
    Ar1 = (A1 / 2.0) * (1.0 - S1)
    Al1 = (A1 / 2.0) * (1.0 + S1)
    Ar2 = (A2 / 2.0) * (1.0 - S2)
    Al2 = (A2 / 2.0) * (1.0 + S2)

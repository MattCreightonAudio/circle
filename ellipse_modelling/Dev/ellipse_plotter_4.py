...
initial_omega_ratio = 2.0 # omega2 / omega1

# Amplitudes (4 parameters: Base, E1 Ratio, E2 Ratio, Total Ratio)
# NEW DEFAULTS: Amp ratios = 1.0, Size Ratio = 1.0. 
# This means: A_r1=0.5, A_l1=0.5, A_r2=0.5, A_l2=0.5
initial_A_Base = 0.5   # A_l1
initial_R_A1 = 1.0     # A_r1 / A_l1 
initial_R_A2 = 1.0     # A_r2 / A_l2 
initial_R_Total_Prime = 1.0  # (A_r2+A_l2) / (A_r1+A_l1) - INVERTED for zero handling
...
initial_D_phi_1 = np.pi / 2    # phi_l1 - phi_r1
initial_D_phi_Total = 0.0      # phi_r2 - phi_r1 (Keeping this zero by default)
initial_D_phi_2 = np.pi / 2    # phi_l2 - phi_r2
...
ax_R_Total_Prime = plt.axes([0.15, 0.28, 0.65, 0.03]) # Renamed axis variable
...
# Freq Ratio step set to 0.5 as requested
slider_omega_ratio = Slider(ax=ax_omega_ratio, label='$\omega_2 / \omega_1$ (Freq Ratio)', valmin=0.1, valmax=5.0, valinit=initial_omega_ratio, valstep=0.5)

# Amplitudes (keeping maxes somewhat conservative to fit in the fixed plot area)
slider_A_Base = Slider(ax=ax_A_Base, label='$A_{l1}$ (Base Amp)', valmin=0.0, valmax=1.5, valinit=initial_A_Base)
slider_R_A1 = Slider(ax=ax_R_A1, label='$R_{A1}=A_{r1}/A_{l1}$ (E1 Amp Ratio)', valmin=0.1, valmax=3.0, valinit=initial_R_A1)
slider_R_A2 = Slider(ax=ax_R_A2, label='$R_{A2}=A_{r2}/A_{l2}$ (E2 Amp Ratio)', valmin=0.1, valmax=3.0, valinit=initial_R_A2)
# Size Ratio valmin set to 0.0 as requested
slider_R_Total_Prime = Slider(ax=ax_R_Total_Prime, label='$R_{Total}=\Sigma A_2/\Sigma A_1$ (Size Ratio)', valmin=0.0, valmax=3.0, valinit=initial_R_Total_Prime) # Renamed slider object
...
def update(val):
    ...
    # Note: Using max(0.001, ...) prevents issues if valmin=0 is ever used for a ratio
    omega2 = slider_omega_base.val * max(0.001, slider_omega_ratio.val) 
    
    A_Base = slider_A_Base.val
    R_A1 = max(0.001, slider_R_A1.val)
    R_A2 = max(0.001, slider_R_A2.val)
    R_Total_Prime = max(0.0, slider_R_Total_Prime.val) # Renamed variable and use max(0.0, ...)
    ...
    # A_Sum2 = A_l2 * (R_A2 + 1)
    Al2 = A_Sum2 / (R_A2 + 1.0) if (R_A2 + 1.0) > 0 else 0.0 # Safety check
    Ar2 = A_Sum2 - Al2
...
# Using the new slider object name

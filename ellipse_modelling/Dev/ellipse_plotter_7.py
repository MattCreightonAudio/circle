...
initial_A_Sum1 = 0.5   # Total Amplitude of E1 (A_r1 + A_l1).
initial_P1 = 0.0       # E1 Shape parameter (0.0 = Line)
initial_P2 = 0.0       # E2 Shape parameter (0.0 = Line)
initial_R_Total_Prime = 1.0  # (A_Sum2 / A_Sum1) - Set to 1.0 to start with equal scale E1 and E2

# Phases (Now 3 relative phases + 1 rotation phase)
initial_Phi_Rot = 0.0          # New global rotation (applied to all 4 components)
initial_D_phi_1 = np.pi / 2    # phi_l1 - phi_r1 (E1 Internal Phase Diff)
initial_D_phi_Total = 0.0      # phi_r2 - phi_r1 (Inter-Ellipse Alignment)
initial_D_phi_2 = np.pi / 2    # phi_l2 - phi_r2 (E2 Internal Phase Diff)

# Create the figure and the main plot axes
# We use subplot_adjust to make room for 9 sliders
fig, ax = plt.subplots()
...
# Inversion for Phases (phi_r1 is fixed to 0, all others are relative or rotated)
# The rotation phase is applied globally to all 4 components.
Phi_Rot = initial_Phi_Rot
D_phi_1 = initial_D_phi_1
D_phi_Total = initial_D_phi_Total
D_phi_2 = initial_D_phi_2

# Underlying Phases (phi_r1 is the base 0 + rotation)
phi_r1 = Phi_Rot
phi_l1 = Phi_Rot + D_phi_1
phi_r2 = Phi_Rot + D_phi_Total
phi_l2 = phi_r2 + D_phi_2 # phi_l2 is based on phi_r2 and D_phi_2
...
# --- 3. Create the UI Sliders (Now 9 Sliders Total) ---

# Define axes for the 9 sliders [left, bottom, width, height]
...
# Phases (3 internal + 1 rotation = 4)
# Re-using the lowest axes for the new rotation slider
ax_Phi_Rot = plt.axes([0.15, 0.22, 0.65, 0.03]) # New Total Rotation Slider
ax_D_phi_1 = plt.axes([0.15, 0.18, 0.65, 0.03])
ax_D_phi_Total = plt.axes([0.15, 0.14, 0.65, 0.03]) # Moved up
ax_D_phi_2 = plt.axes([0.15, 0.10, 0.65, 0.03]) # Moved up
...
slider_omega_ratio = Slider(ax=ax_omega_ratio, label='$\omega_2 / \omega_1$ (Freq Ratio)', valmin=1.0, valmax=5.0, valinit=initial_omega_ratio, valstep=0.5) 

# Amplitudes (P1 and P2 range from -1 to 1)
slider_A_Sum1 = Slider(ax=ax_A_Sum1, label='$\Sigma A_1$ (E1 Total Amp)', valmin=0.0, valmax=1.5, valinit=initial_A_Sum1) 
...
slider_R_Total_Prime = Slider(ax=ax_R_Total_Prime, label='$R_{Total}=\Sigma A_2/\Sigma A_1$ (Size Ratio)', valmin=0.0, valmax=3.0, valinit=initial_R_Total_Prime) 

# Phases (Angles in Radians)
slider_Phi_Rot = Slider(ax=ax_Phi_Rot, label='$\Phi_{Rot}$ (Total Rotation)', valmin=-np.pi, valmax=np.pi, valinit=initial_Phi_Rot)
slider_D_phi_1 = Slider(ax=ax_D_phi_1, label='$\Delta\phi_1$ (E1 Internal Phase Diff)', valmin=-np.pi, valmax=np.pi, valinit=initial_D_phi_1)
slider_D_phi_Total = Slider(ax=ax_D_phi_Total, label='$\Delta\phi_{Total}$ (E2 CW Alignment)', valmin=-np.pi, valmax=np.pi, valinit=initial_D_phi_Total)
slider_D_phi_2 = Slider(ax=ax_D_phi_2, label='$\Delta\phi_2$ (E2 Internal Phase Diff)', valmin=-np.pi, valmax=np.pi, valinit=initial_D_phi_2)
...
def update(val):
    # Get current values from all 9 sliders
    ...
    Phi_Rot = slider_Phi_Rot.val          # New Rotation
    ...
    # The 'Total Rotation' phase shifts all four components by the same amount,
    # thereby rotating the final figure without changing its internal structure.
    # The remaining three Delta phases define the internal alignment.
    
    phi_r1 = Phi_Rot                   # Base Phase is now the Rotation
    phi_l1 = Phi_Rot + D_phi_1         # E1 Left component
    
    phi_r2 = Phi_Rot + D_phi_Total     # E2 Right component (controlled by Inter-Ellipse alignment)
    phi_l2 = phi_r2 + D_phi_2          # E2 Left component (controlled by E2 Internal Phase)
...
# New phase connection order
slider_Phi_Rot.on_changed(update)
